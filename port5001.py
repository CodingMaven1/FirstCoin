...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, reciever = 'Myself', amount=1)
    block = blockchain.create_block(proof,previous_hash)
    response = {'message': 'Hey!, You just mined a block',
                'index': block['index'],
...
def add_transaction():
    ...
    response = {'message': f'Will be Inserted at block {index}'}
...
def replace_chain():
    ...
    if replace_chain:
        response= {'message': 'Nodes had different chains, so this is updated chain',
                   'replace_chain': blockchain.chain}
    else:
        response= {'message': 'This is the largest chain',
